...
func New(cfg *config.Config) (*BpfObjects, error) {
	if err := IncreaseResourceLimits(); err != nil {
		logger.Fatalf("Can't increase resource limits: %s", err.Error())
	}

	b := &BpfObjects{
		FarIdTracker: NewIdTracker(cfg.FarMapSize),
		QerIdTracker: NewIdTracker(cfg.QerMapSize),
	}

	if err := b.Load(); err != nil {
		return nil, errors.New(fmt.Sprintf("Loading bpf objects failed: %s", err.Error()))
	}

	if cfg.EbpfMapResize {
		if err := b.ResizeAllMaps(cfg.QerMapSize, cfg.FarMapSize, cfg.PdrMapSize); err != nil {
			return nil, errors.New(fmt.Sprintf("Failed to set ebpf map sizes: %s", err))
		}
	}
