...
func New(cfg *config.Config, mapOperations ebpf.ForwardingPlaneController) (*PfcpConnection, error) {

	// Create PFCP connection
	var pfcpHandlerMap = PfcpHandlerMap{
	...
	udpAddr, err := net.ResolveUDPAddr("udp", cfg.PfcpAddress)
	if err != nil {
		logger.Panicf("Can't resolve UDP address: %s", err.Error())
		return nil, err
	}
	udpConn, err := net.ListenUDP("udp", udpAddr)
	if err != nil {
		logger.Infof("Can't listen UDP address: %s", err.Error())
		return nil, err
	}

	n3Addr := net.ParseIP(cfg.N3Address)
	if n3Addr == nil {
		return nil, fmt.Errorf("failed to parse N3 IP address ID: %s", cfg.N3Address)
	}
	logger.Infof("Starting PFCP connection: %v with Node ID: %v and N3 address: %v", udpAddr, cfg.PfcpNodeId, n3Addr)
...
func (connection *PfcpConnection) Run() {
	go func() {
		for {
			...
			time.Sleep(time.Duration(connection.cfg.HeartbeatInterval) * time.Second)
		}
	}()
	buf := make([]byte, 1500)
	for {
		...
		if err != nil {
			logger.Infof("Error reading from UDP socket: %s", err.Error())
			time.Sleep(1 * time.Second)
			continue
		}
		logger.Infof("Received %d bytes from %s", n, addr)
...
func (connection *PfcpConnection) Handle(b []byte, addr *net.UDPAddr) {
	...
	if err != nil {
		logger.Infof("Error handling PFCP message: %s", err.Error())
	}
}
...
func (connection *PfcpConnection) RefreshAssociations() {
	for assocAddr, assoc := range connection.NodeAssociations {
		if assoc.IsExpired() {
			logger.Infof("Pruning expired node association: %s", assocAddr)
			connection.DeleteAssociation(assocAddr)
		}
	}
	for _, assoc := range connection.NodeAssociations {
		if !assoc.IsHeartbeatScheduled() {
			assoc.ScheduleHeartbeatRequest(time.Duration(connection.cfg.HeartbeatTimeout)*time.Second, connection)
		}
	}
}
...
func (connection *PfcpConnection) DeleteAssociation(assocAddr string) {
	...
	for sessionId, session := range assoc.Sessions {
		logger.Infof("Deleting session: %d", sessionId)
