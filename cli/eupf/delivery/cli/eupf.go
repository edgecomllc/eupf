...
func NewCLI(usecase domain.EupfUseCase) *CLI {
	...
	configCmd.AddCommand(cli.newConfigSetBaseURLCmd())
	configCmd.AddCommand(cli.newConfigShowBaseURLCmd())
...
Example:
  eupf config set-baseurl --url http://localhost:8081/api/v1`,
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err := cli.usecase.ConfigSetNewEUPFBaseURL(newBaseURL); err != nil {
	...
	cmd.Flags().StringVar(&newBaseURL, "url", "", "New base URL to save in config (e.g. http://localhost:8081/api/v1)")
	return cmd
}
...
which is stored in the config-cli.yaml file under "eupf_addr".`,
		Run: func(cmd *cobra.Command, args []string) {
			url, err := cli.usecase.ConfigShowEUPFBaseURL()
			if err != nil {
